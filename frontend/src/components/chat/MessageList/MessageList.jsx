...
const MessageList = ({ messages, currentUser, uploadingFile }) => {
            ...
            {messages.map((msg, index) => {
                ...
                return (
                    <React.Fragment key={index}>
                        ...
                        <MessageBubble
                            message={msg}
                            isMe={msg.sender === currentUser?.username || msg.sender === 'optimistic_sender'}
                        />
                    </React.Fragment>
                );
            })}

            {/* Hiển thị trạng thái đang gửi file (Optimistic UI) */}
            {uploadingFile && (
                <div className="message-row me">
                    <div className="message-content text-msg me" style={{ opacity: 0.6 }}>
                        <span style={{ fontStyle: 'italic' }}>⏳ Đang tải file lên...</span>
                    </div>
                </div>
            )}
